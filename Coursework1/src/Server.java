            ...
            while (true) {
                ...
                try (DataInputStream din = new DataInputStream(clientSocket.getInputStream());
                    ...
                    while (continueCommunication) {
                        // Ask the client if they want to send a message
                        dout.writeUTF("Do you want to send a message? [y/n]");
                        dout.flush();

                        String clientResponse = din.readUTF(); // Expecting 'y' or 'n'

                        if ("y".equalsIgnoreCase(clientResponse)) {
                            // Read the message length and then the message
                        ...
                        } else if ("n".equalsIgnoreCase(clientResponse)) {
                            continueCommunication = false; // Client does not want to send messages
                        } else {
                            dout.writeUTF("Invalid response. Please type 'y' or 'n'.");
                            dout.flush();
                        }
                    }
