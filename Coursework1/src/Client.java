...
//            dout.writeUTF(hashedUserID); // Send hashed UserID
//            dout.flush();

//            int messageCount = din.readInt(); // Read the number of messages
//            System.out.println("Number of messages stored for you: " + messageCount);
            ...
            System.out.print("> ");
            BufferedReader br = new BufferedReader(new InputStreamReader(System.in)); //read user input
...
//            if (din.available() > 0) { // Check if server sent any data
//                int responseLength = din.readInt(); // Read response length
//                if (responseLength > 0) {
//                    byte[] encryptedResponse = new byte[responseLength];
//                    din.readFully(encryptedResponse); // Read the encrypted response from the server
//
//                    // Decrypt the response
//                    Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding");
//                    cipher.init(Cipher.DECRYPT_MODE, clientPrivateKey); // Use client's private key to decrypt
//                    byte[] decryptedResponse = cipher.doFinal(encryptedResponse);
//
//                    // Convert bytes to string and display the server response
//                    String serverResponse = new String(decryptedResponse);
